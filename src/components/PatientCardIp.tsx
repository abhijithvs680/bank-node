...
export const PatientCardIp = ({ patient, patientType = 'inpatient' }: PatientCardProps) => {
  ...
  return (
    ...
    >
      <Card
        className="p-5 border-t-4 border-t-blue-600 flex flex-col h-full bg-white hover:shadow-2xl hover:-translate-y-1 transition-all duration-300 rounded-[24px] group border-x-0 border-b-0 shadow-sm text-left"
      >
        {/* Top Section */}
        <div className="flex items-start justify-between mb-3 gap-3">
          <div className="flex-1 min-w-0">
            <h3 className="text-[1.125rem] font-bold text-slate-800 truncate group-hover:text-blue-600 transition-colors">
              {dealName}
            </h3>
            <div className="flex items-center gap-2 mt-0.5">
              <span className="text-[0.75rem] font-semibold text-slate-400 uppercase tracking-wider">ID: {dealId}</span>
            </div>
          </div>
        ...
        <div className="space-y-3 mb-4 flex-1">
          ...
          <div className="grid grid-cols-2 gap-2">
            <div className="p-2 border border-slate-100 rounded-[10px] text-center">
              <span className="text-[10px] font-bold text-slate-400 uppercase block">Jurisdiction</span>
              <span className="text-[13px] font-bold text-slate-700">{jurisdiction}</span>
            </div>
            <div className="p-2 border border-slate-100 rounded-[10px] text-center">
