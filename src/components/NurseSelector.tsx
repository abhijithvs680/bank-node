...
interface NurseSelectorProps {
  currentNurse: string;
  admissionId: string;
  onStaffUpdated?: (newNurse: string) => void;
}

export const NurseSelector = ({
  currentNurse,
  admissionId,
  onStaffUpdated
  ...
  const [selectedNurse, setSelectedNurse] = useState(currentNurse);
  ...
  const handleSave = async () => {
    setIsLoading(true);
    try {
      // In a real app, this would be an API call to update the patient's nurse
      //  await new Promise(resolve => setTimeout(resolve, 500));
      apiService.switchStaffRole(admissionId, 'nurse', getCurrentNurseId());
      toast.success('Nurse updated successfully');
      onStaffUpdated?.(selectedNurse);
      setIsOpen(false);
    } catch (error) {
      toast.error('Failed to update nurse');
      setSelectedNurse(currentNurse); // Revert on error
    } finally {
      setIsLoading(false);
    }
  };
  const handleEdit = async () => {
    try {
      const response = await apiService.switchStaffRole(admissionId, 'NurseSelector', "");
      setAssignedNurseList(JSON.parse((response as any)?.Nurse || "[]"));
    } catch (error) {
  ...
  const getCurrentNurseName = () => {
    const nurse = assignedNurseList.find(n => n.Name === selectedNurse || n.id === selectedNurse);
    return nurse ? nurse.Name : selectedNurse;
  };

  const getCurrentNurseUnit = () => {
    const nurse = assignedNurseList.find(n => n.Name === selectedNurse || n.id === selectedNurse);
    return nurse ? nurse.unit : '';
  };
        ...
        <Card className="relative medical-card fade-in">
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-1 pt-3">
            <CardTitle className="flex items-center gap-1 text-xs font-medium">
              ...
              {currentNurse ? "Nurse" : "Add Nurse"}
            </CardTitle>
          </CardHeader>
          <CardContent className="pt-0 pb-2 px-4">
            <div className="text-xs font-medium pl-6">{getCurrentNurseName()}</div>
            <div className="text-xs text-muted-foreground pl-6">{getCurrentNurseUnit()} Unit</div>
          </CardContent>
          {isLoading && (
        ...
        <DialogContent className="sm:max-w-md rounded-[24px] border-[#e0e3f5] p-0 overflow-hidden shadow-2xl [&>button]:hidden">
          ...
          <div className="p-6 space-y-5 bg-white">
            <div className="space-y-2">
              <Label htmlFor="nurse" className="block text-[14px] font-bold text-[#1a2256] mb-2">
                Nurse
              </Label>
              <Select value={selectedNurse} onValueChange={setSelectedNurse}>
                ...
                <SelectContent className="rounded-[16px] border-[#e0e3f5] shadow-xl">
                  {assignedNurseList.map((nurse) => (
                    <SelectItem key={nurse.id} value={nurse.Name}>
                      <div className="flex flex-col">
                        <span className="font-bold text-[#1a2256]">{nurse.Name}</span>
                        <span className="text-[11px] text-[#6e6868] font-medium">{nurse.unit} Unit</span>
                      </div>
                    </SelectItem>
                  ))}
                </SelectContent>
